...
<head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css"
        integrity="sha512-Fo3rlrZj/kTcB+K0G3Lqf0ZCwY1gK1dR1U1VnKhT2oVhCF0wYlSZeJkN+U5U3dQe1J6/0N9l1f6Z7m6ITX6uHg=="
        ...
        <footer id="htc__footer">
                ...
                <div class="container">
                    <div class="row">
                                ...
                                <div class="ft__details">
                                    <div class="row">
                                        <div class="col-sm-12" style="display: flex; flex-direction: column; align-items: center;">
                                            <img src="/storage/images_slide/71ZsnD4hn3_OIP.jpg" alt="" style="width: 50%;">
                                        </div>
                                    </div>
                                    <div class="ft__contact" style="margin-top: 20px;">
                                        <div style="display: flex; align-items: center; margin: 5px 0;">
                                            <span style="color:#fff" >Địa chỉ: 682 Sư Vạn Hạnh, P.12, Quận 10, TPHCM</span>
                                        </div>
                                        <div style="display: flex; align-items: center; margin: 5px 0;">
                                            <span style="color:#fff">SĐT: 0902 33 22 18</span>
                                        </div>
                                        <div style="display: flex; align-items: center; margin: 5px 0;">
                                            <span style="color:#fff">Email: comebuyvn@gmail.com</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-md-3 col-sm-6 col-xs-12 xmt-40">
                            <div class="footer">
                                <h2 class="title__line--2">Loại Trà Sữa</h2>
                                <div class="ft__inner">
                                    <ul class="ft__list">
                                        @foreach ($dataCategory as $item)
                                        <li><a
                                                href="/shop/category/{{$item->category_id}}-{{Str::slug($item->category_name, '-')}}.html">{{$item->category_name}}</a>
                                        </li>
                                        @endforeach
                                    </ul>
                                </div>
                        ...
                        <div class="col-md-4 col-sm-6 col-xs-12">
                            <div class="footer">
                                <div class="ft__social__link" style="text-align: center; margin-top: 10px;">
                                ...
                                <div class="ft__map">
                                    <iframe
                                        src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3151.835434509836!2d144.95373631531865!3d-37.8162799797519!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x6ad642af0f11e6b7%3A0x5045675218ceed0!2sYour%20Location!5e0!3m2!1sen!2s!4v1618495075045!5m2!1sen!2s"
            ...
            <div class="fab-wrapper">
                ...
                <div class="fab-wheel">
                    ...
                    <a class="fab-action fab-action-3" onclick="Tawk_API.showWidget();Tawk_API.maximize();"
                        rel="nofollow">
    ...
    $('.cart__menu').click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var cartTotalPrice = 0; // Tổng tiền giỏ hàng
                var groupedItems = {};

                if (Array.isArray(data[0])) {
                    // Duyệt qua từng item để nhóm các sản phẩm giống nhau lại với nhau
                    data[0].forEach(function(item) {
                        // Xử lý topping: nếu có, lấy danh sách tên topping và sắp xếp
                        var toppingNames = '';
                        if (item['topping'] && item['topping'].length > 0) {
                            toppingNames = item['topping']
                                .map(function(t) {
                                    return t.name;
                                })
                                .sort()
                        ...
                        // Tạo key nhóm: dựa trên cart_id, size và toppingNames
                        var key = item['cart_id'] + '-' + item['size'] + '-' + toppingNames;

                        // Nếu chưa có key này trong groupedItems, thêm mới
                        if (!groupedItems[key]) {
                            // Clone item và ép số lượng về số nguyên
                            groupedItems[key] = $.extend(true, {}, item);
                            groupedItems[key]['cart_quantity'] = parseInt(item[
                                'cart_quantity'] || 1);
                        } else {
                    ...
                    // Duyệt qua các item đã nhóm để tạo HTML hiển thị
                    for (var key in groupedItems) {
                        if (groupedItems.hasOwnProperty(key)) {
                            var item = groupedItems[key];

                            // Xử lý topping: lấy danh sách tên topping cho hiển thị
                            var toppingText = (item['topping'] && item['topping'].length > 0) ?
                                'Topping: ' + item['topping'].map(function(t) {
                                    return t.name;
                                }).join(', ') :
                                'Không có topping';

                            // Tính tổng giá topping (nếu có)
                            var toppingTotalPrice = (item['topping'] && item['topping'].length >
                                0) ?
                                item['topping'].reduce(function(sum, topping) {
                                    return sum + parseFloat(topping.price || 0);
                                }, 0) :
                                0;

                            // Giá sản phẩm ban đầu
                            var basePrice = parseFloat(item['cart_price_sale'] || 0);

                            // Nếu size là L thì cộng thêm 7000 đ
                            var extraPrice = 0;
                            if (item['size'] && item['size'].toUpperCase() === 'L') {
                                extraPrice = 7000;
                            }

                            // Tính tổng giá cho sản phẩm: (basePrice + toppingTotalPrice + extraPrice) * số lượng
                            var quantity = parseInt(item['cart_quantity'] || 1);
                            var totalPrice = (basePrice + toppingTotalPrice + extraPrice) *
                            quantity;
                            ...
                            output += `
                                    <div class="shp__single__product tr-${item['cart_id']}">
                                        ...
                                        <div class="shp__pro__details">
                                            <h2>
                                                <a href="/shop/product/${item['cart_id']}">
                                                    ${item['cart_product']}
                                                </a>
                                            </h2>
                                            ...
                                            <span class="shp__price">${formatter.format(totalPrice)} VNĐ</span>
                                        </div>
                                        <form>
                                            @csrf
                                            <div class="remove___btn">
                ...
                // Cập nhật tổng tiền giỏ hàng và hiển thị danh sách sản phẩm
                $('.total__price').text(formatter.format(cartTotalPrice) + ' VNĐ');
                ...
                $('.button_del').click(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(dataDel) {
                            $('.tr-' + id).remove();
                            $('.total__price').text(formatter.format(dataDel[
                                0]) + ' VNĐ');
                        },
                        error: function(xhr) {
